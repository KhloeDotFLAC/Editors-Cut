...
class Cache extends MusicBeatState
{
	...
	override function create()
	{
		...
		shitz = new FlxText("Loading...");
		shitz.scrollFactor.set();
		shitz.setFormat("VCR OSD Mono", 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		shitz.setPosition(FlxG.width - shitz.width - 50, FlxG.height - shitz.height - 20);
		add(shitz);

		#if cpp
		for (i in FileSystem.readDirectory(FileSystem.absolutePath("assets/shared/images/bedroom")))
		{
			if (!i.endsWith(".png"))
				continue;
			images.push(i);
	...
	function cache()
	{
		#if !linux
			var sound1:FlxSound;
			sound1 = new FlxSound().loadEmbedded(Paths.voices('caffeine'));
			sound1.play();
			sound1.volume = 0.00001;
			FlxG.sound.list.add(sound1);

			var sound2:FlxSound;
			sound2 = new FlxSound().loadEmbedded(Paths.inst('caffeine'));
			sound2.play();
