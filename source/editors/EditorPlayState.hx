			...
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					if(daStrumTime >= startPos) {
						var daNoteData:Int = Std.int(songNotes[1] % Main.ammo[PlayState.mania]);

						var gottaHitNote:Bool = section.mustHitSection;

						if (songNotes[1] > Main.ammo[PlayState.mania] - 1)
						{
	...
	override function update(elapsed:Float) {
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				var swagWidth = Note.swidths[0] * Note.scales[PlayState.mania];
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					StrumPlayAnim(true, Std.int(Math.abs(daNote.noteData)) % Main.ammo[PlayState.mania], time);
