...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (Main.isDuet == true) {
			keko = new Boyfriend(100, 100, "keko");
		}

		boyfriend = new Boyfriend(770, 450, SONG.player1);

		if (Main.isDuet == true) {
			keko.setPosition(boyfriend.x - 50, boyfriend.y - 260);
		}
		...
		add(healthBarBG);
			...
			var healthCol = 0xFFFF0000;

			switch (SONG.player2)
			{
				//Custom healthbar colors per character
				case 'gf':
					healthCol = 0xFFA5004D;
				case 'keko':
					healthCol = 0xFFC74686;
				case 'dad' | 'pshaggy':
					healthCol = 0xFF33724A;
				default:
					healthCol = FlxColor.WHITE;
			}

		healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			'health', 0, 2);
		healthBar.scrollFactor.set();
		healthBar.createFilledBar(healthCol, 0xFF31B0D1);
		// healthBar
		add(healthBar);
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.SPACE && !isKekoSinging && Main.isDuet == true)
		{
			isKekoSinging = true;
		} 
		else if (FlxG.keys.justPressed.SPACE && isKekoSinging == true)
		{
			isKekoSinging = false;
		}
		...
		function goodNoteHit(note:Note):Void
			{
				...
				if (!note.wasGoodHit)
				{
					...
					if (isKekoSinging != true)
					{
						boyfriend.playAnim('sing' + sDir[note.noteData], true);
					}
					else
					{
						keko.playAnim('sing' + sDir[note.noteData], false);
					}
