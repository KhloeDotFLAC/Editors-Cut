...
class StrumNote extends FlxSprite
{
	...
	public function new(x:Float, y:Float, leData:Int, player:Int) {
		...
		else
		{
			...
			x += Note.swidths[PlayState.mania] * Note.swagWidth * Math.abs(noteData);
				
			var dirName = Main.gfxDir[Main.gfxHud[PlayState.mania][noteData]];
			var pressName = Main.gfxLetter[Main.gfxIndex[PlayState.mania][noteData]];
	...
	public function postAddedToGroup() {
		playAnim('static');
		x += Note.swidths[PlayState.mania] * Note.scales[PlayState.mania] * noteData;
		x += 50 - Note.posRest[PlayState.mania];
